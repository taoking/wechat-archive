...
public final class WeChatArchiveV1Database: @unchecked Sendable {
    ...
    public func finishImportRun(_ id: String, status: ArchiveV1ImportStatus, sourceDatabaseCount: Int, summary: ArchiveV1ImportSummary) throws {
        ...
        try bind(Int64(summary.rawDATArchived + summary.missingLocalMedia), at: 4, to: statement)
    ...
    public func insertMessage(
        ...
        if let id = try messageID(sourceDatabase: sourceDatabase, sourceTable: sourceTable, rowIdentifier: sourceRowIdentifier) {
            return (id, false)
        }
        let id = UUID().uuidString.lowercased()
        try inTransaction {
            let statement = try prepare("""
                INSERT INTO messages (id, conversation_id, source_database, source_table, source_row_identifier, source_local_id, source_server_id, timestamp, sender_source_id, receiver_source_id, raw_local_type, normalized_type, text_content, reply_source_id, source_sequence, imported_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """)
            defer { sqlite3_finalize(statement) }
            try bind(id, at: 1, to: statement)
            ...
            try bind(sourceRowIdentifier, at: 5, to: statement)
    ...
    public func insertMediaAsset(
        ...
        let sourceKey = "\(messageID):\(variant.rawValue)"
        ...
        let statement = try prepare("""
            INSERT INTO media_assets (id, source_key, media_type, variant, source_format, decoded_format, status, raw_archive_path, decoded_archive_path, raw_size, decoded_size, width, height, sha256_raw, sha256_decoded, source_file_base, created_at)
            VALUES (?, ?, 'image', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(source_key) DO UPDATE SET
              source_format = excluded.source_format, decoded_format = excluded.decoded_format, status = excluded.status,
              raw_archive_path = excluded.raw_archive_path, decoded_archive_path = excluded.decoded_archive_path,
              raw_size = excluded.raw_size, decoded_size = excluded.decoded_size, width = excluded.width, height = excluded.height,
        ...
        try bind(variant.rawValue, at: 3, to: statement)
        try bind(sourceFormat, at: 4, to: statement)
        try bind(decodedFormat, at: 5, to: statement)
        try bind(status.rawValue, at: 6, to: statement)
        try bind(rawArchivePath, at: 7, to: statement)
        try bind(decodedArchivePath, at: 8, to: statement)
        try bind(rawSize, at: 9, to: statement)
        try bind(decodedSize, at: 10, to: statement)
        try bind(width.map(Int64.init), at: 11, to: statement)
        try bind(height.map(Int64.init), at: 12, to: statement)
        try bind(rawSHA256, at: 13, to: statement)
        try bind(decodedSHA256, at: 14, to: statement)
        try bind(sourceFileBase, at: 15, to: statement)
        try bind(Date().timeIntervalSince1970, at: 16, to: statement)
        guard sqlite3_step(statement) == SQLITE_DONE else { throw ArchiveError.databaseFailure }
    ...
    public func sourceValues(sourceDatabase: String, sourceTable: String, rowIdentifier: String) throws -> [String: ArchivedSQLiteValue] {
        guard let messageID = try messageID(sourceDatabase: sourceDatabase, sourceTable: sourceTable, rowIdentifier: rowIdentifier) else { return [:] }
    ...
    public func reconstruction() throws -> [ArchiveV1ReconstructedMessage] {
        let statement = try prepare("""
            ...
            ORDER BY m.timestamp ASC, m.source_sequence ASC, m.source_database ASC, m.source_table ASC, m.source_row_identifier ASC, a.variant ASC
            """)
        defer { sqlite3_finalize(statement) }
    ...
    private func migrate() throws {
        ...
        try inTransaction {
            ...
            try execute("""
                CREATE TABLE messages (id TEXT PRIMARY KEY, conversation_id TEXT NOT NULL REFERENCES conversations(id), source_database TEXT NOT NULL, source_table TEXT NOT NULL, source_row_identifier TEXT NOT NULL, source_local_id INTEGER, source_server_id INTEGER, timestamp INTEGER NOT NULL, sender_source_id TEXT, receiver_source_id TEXT, raw_local_type INTEGER, normalized_type TEXT NOT NULL, text_content TEXT, reply_source_id TEXT, source_sequence INTEGER NOT NULL, imported_at REAL NOT NULL, UNIQUE(source_database, source_table, source_row_identifier))
                """)
            try execute("CREATE INDEX messages_reconstruction ON messages(timestamp, source_sequence, source_database, source_table, source_row_identifier)")
            try execute("""
    ...
    private func messageID(sourceDatabase: String, sourceTable: String, rowIdentifier: String) throws -> String? {
        let statement = try prepare("SELECT id FROM messages WHERE source_database = ? AND source_table = ? AND source_row_identifier = ?")
        defer { sqlite3_finalize(statement) }
        try bind(sourceDatabase, at: 1, to: statement); try bind(sourceTable, at: 2, to: statement); try bind(rowIdentifier, at: 3, to: statement)
